...
@section('breadcrumb')
    <h3 class="page-title">تقييمات {{$data->title}}</h1>
        @endsection
                        ...
                        <div class="card-body">
                            ...
                            <table id="datatable" class="table table-bordered dt-responsive nowrap" cellspacing="0"
                                ...
                                @if($data->Reviews->count() > 0)
                                    @foreach ($data->Reviews as $item => $row)
                                        <tr>
                                            <td>{{$item+1}}</td>
                                            @php
                                                $span = '';
                                                for ($star = 0; $star < 5; $star++){
                                                    if (($row->rate > $star) && ($row->rate >= $star+1)){
                                                         $span .= '<span class="fa fa-star star-color"></span>';
                                                    }elseif (($row->rate > $star) && ($row->rate < $star+1)){
                                                         $span .= '<span class="fa fa-star-half-full star-color"></span>';
                                                    }else{
                                                         $span .= '<span class="fa fa-star-o fa-lg"></span>';
                                                    }
                                                }
                                            @endphp
                                            <td>{!! $span !!}</td>
